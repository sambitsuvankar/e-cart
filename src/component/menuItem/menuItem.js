...
const MenuItems = ({id, image, category, size}) => {
    ...
    return(
        <div key={id} onClick={()=> navigate(`shop/${category}`)} className={`bg-white w-auto h-60 sm:h-3/5 m-1 rounded-2xl flex flex-col content-center shadow-2xl justify-center cursor-pointer sm:col-span-1 overflow-hidden  ${size == 'large' ? 'col-span-4' : 'col-span-2'}`}>
